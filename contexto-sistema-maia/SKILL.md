...
|---|---|---|---|---|
| brief-strategy | Brief Maker | Primer filtro: convierte documentos raw en Golden Briefing | `golden_briefing_v<N>.json` + `.docx` + `one_pager.html` + `formulario_area.docx` | `brief-quality-rubric` |
| media-strategy | Mix Media Planner | Estrategia por canal y territorio: priorizacion territorial, tier, canales, comentarios expertos, tablas Movistar, etiquetado de inferencias | `media_strategy_v<N>.json` + `.docx` + `.html` | `rol-medios-movistar` |
| campaign-builder | Creative Strategist | Estrategia creativa en 3 niveles: marco estrategico, campanas por territorio, profundidad por pieza | `campaign_strategy_v<N>.json` + `.docx` + `.html` | (ninguna exclusiva) |
| campaign-design | Visual Designer | Mockups multi-formato por pieza (HTML, SVG, composiciones) | Mockups + `<slug>_design_rationale.docx` | `html-component-library`, `brand-visual-composition-movistar`, `brand-typography-movistar` |
| director | Campaign Manager | Cierre: checklist V01-V16, resumen ejecutivo, Campaign Kit | Resumen de cierre + Campaign Kit | `validacion-maia-checklist`, `journey-canales-movistar` |
...
- **C produce, humano aprueba o itera** (Gate C). El humano aprueba y C pasa directamente a D.
- **D produce, humano aprueba** (Gate D), y luego escala a **Cierre (Campaign Manager)**. El Campaign Manager ejecuta la checklist V01-V16 sobre el paquete completo, genera el resumen ejecutivo, consolida todos los outputs en un Campaign Kit entregable y activa la revision humana final.
...
- **visual-**: reglas y componentes para la produccion visual del Visual Designer.
...
| `validacion-maia-checklist` | Checklist V01-V16 de cierre del Campaign Manager | Campaign Manager | active |
...
#### Playbooks por canal (5, carga condicional)
...
| **B** | `golden-briefing-schema`, `campaign-output-format`, `communication-tiers-movistar`, `btl-tone-movistar`, `product-verticals-movistar`, `tesis-estrategica-movistar`, `rol-medios-movistar`, `matriz-objetivo-canal`, `reglas-planner-movistar`, `contexto-sistema-maia` | Playbooks de los canales activos |
| **C** | `golden-briefing-schema`, `campaign-output-format`, `brand-voice-movistar`, `estilo-terminologia-movistar`, `copywriting-principles-movistar`, `communication-tiers-movistar`, `btl-tone-movistar`, `product-verticals-movistar`, `contexto-sistema-maia` | Playbooks de los canales activos |
...
| **Campaign Manager** | `campaign-output-format`, `golden-briefing-schema`, `validacion-maia-checklist`, `brand-voice-movistar`, `communication-tiers-movistar`, `tesis-estrategica-movistar`, `matriz-objetivo-canal`, `reglas-planner-movistar`, `journey-canales-movistar`, `brand-visual-guidelines-movistar`, `contexto-sistema-maia` | -- |
